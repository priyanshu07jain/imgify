...
const {genrateImage}=useContext(AppContext)
  ...
  return (
    ...
    {!isImageLoaded&& <div className=' flex w-full max-w-xl bg-neutral-500 text-white text-sm p-0.5 mt-10 rounded-full'>
      <input onChange={e=>setInput(e.target.value)} placeholder='Describe what you want to genrate' className=' flex-1 bg-transparent 
      outline-none  ml-8 max-sm:w-20 placeholder-color' />
      <button type='submit' className='bg-zinc-900  px-10 sm:px-16 py-3 rounded-full'>Generate</button>
    </div>}
   {isImageLoaded&& <div className="flex flex-wrap gap-2.5 justify-center text-center text-white text-sm p-0.5 mt-10 rounded-full" >
      <p className="bg-transparent border border-zinc-900 text-black px-8 py-3 rounded-full cursor-pointer" onClick={(()=>{setisImageLoaded(false), setImage(assets.sample_img_1)})}>Generate Another</p>
